...
public class CollectionsPanel extends MyPanel {
    ...
    private void init() {
        ...
        if (decksEditor == null)
            decksEditor = new DecksEditor();

        info = new InfoPanel(true);
    ...
    private void makeTabs() {
        ...
        ModelViewerScroll naturalViewer = new ModelViewerScroll(new AddToDeckAction(), false, false,
                true, false);
        naturalViewer.setModels(PlayerMapper.getInstance().filteredCards(Card.getAllCardsOfClass(Class.Natural),
        ...
        for (Heroes hero : Heroes.getAllHeroes()) {
            ModelViewerScroll heroCardsViewer = new ModelViewerScroll(new AddToDeckAction(), false, false, true, false);
            heroCardsViewer.setModels(PlayerMapper.getInstance().filteredCards(hero.getAllCardsOfThisClass(),
                    filterPanel.getManaNumber(), filterPanel.getSearchText(), filterPanel.getOwnedCardsOnly()));
    ...
    public static void showGoToShopOption() {
    ...
    private static class AddToDeckAction extends MyButtonAction {
...
//            PlayerLogger.writeLog("Add to deck button clicked", "card : " + getModel().getName());
            PlayerMapper.getInstance().getLogger().writeLog(LogType.BUTTON_CLICK, "Add to deck button -> " + getModel().getName());
            PlayerMapper.getInstance().addCardToDeck((Card) getModel());
...
//        PlayerLogger.writeLog("Apply filter button clicked", "Mana : " + manaNumber + ", Search text : " +
//                searchText + ", Only Owned Card : " + ownedCardsOnly);
        PlayerMapper.getInstance().getLogger().writeLog(LogType.BUTTON_CLICK, "Apply Filter -> " + "Mana : " +
