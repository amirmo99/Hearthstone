...
public class GameActionExecutor {
    ...
    public GameActionExecutor(MyGameState gameState) {
        this.gameState = gameState;
        this.constants = new LogicConstants();
        this.monitor = new Monitor();

        this.effectsVisitor = new MyGameVisitor(gameState, this, GameMapper.getInstance().getLogger());
    ...
    private void writeLog(LogType type, String message) {
        GameMapper.getInstance().getLogger().writeLog(type, message);
    }

    private void makeTimer() {
        gameState.getTimer().setUsualRunnable(this::endTurn);
        gameState.getTimer().setStaticRunnable(this::updateGraphics);
    }
...
//        PlayerLogger.writeLog("game begins");
    ...
    private void getPassive(int playerIndex) {
        InfoPassive passive = GameMapper.getInstance().askForPassive(initialPassives());
        if (playerIndex == 1)
...
//        PlayerLogger.writeLog("Passive Activated", "Passive : " + passive.getName());
    ...
    private void letPlayerChangeInitialCards() {
        ArrayList<Card> cards = new ArrayList<>();
        for (int i = 0; i < constants.getStartingCardsNumber(); i++) {
            cards.add(gameState.getPlayer1().getDeck().get(i));
        }

        String message = "Select to remove card from starting deck.";
        ArrayList<Card> toBeRemovedCards = GameMapper.getInstance().askForMultipleCards(cards, message);

        changeStartingCards(toBeRemovedCards);
    }

    private void changeStartingCards(ArrayList<Card> cards) {
        PlayerFields player = gameState.getPlayer1();
        ...
        synchronized (this) {
            if (!gameState.isGameOver()) {
                for (Minion minion : gameState.getActivePlayer().getPlayedCards()) {
                    if (minion != null) {
                        gameState.pushAffectingModel(minion);
                        minion.getCardEffects().getOnTurnEnd().accept(effectsVisitor);
                        gameState.popAffectingModel();
                    }
                }
...
//        PlayerLogger.writeLog("Shuffling Cards");
    ...
    private void performOnCardReceiveEffect(Card card, boolean activePlayer) {
        ...
        for (GameModel model : getAllModels(activePlayer)) {
            gameState.pushAffectingModel(model);
    ...
    public void playCard(Card card, int index) {
        if (!canPlayCard(card).equalsIgnoreCase("Success")) {
            GameMapper.getInstance().showError(canPlayCard(card));
            return;
        }
    ...
    private void performBattleCry(Card card) {
        gameState.pushAffectingModel(card);
    ...
    private void killWeapon(Weapon weapon, Heroes hero) {
        gameState.pushAffectingModel(weapon);
    ...
    public Card playerDiscover(List<Card> cards) {
        return GameMapper.getInstance().discover(cards);
    }

    private void updateGraphics() {
        GameMapper.getInstance().updateGraphics();
    }
        ...
        if (!power.isPassive()) {
            if (power.getMana() <= getManaLeft()) {
                if (power.getActionsLeft() > 0) {
                    ...
                    GameMapper.getInstance().showError("No more actions left.");
                }
            } else
                GameMapper.getInstance().showError("Not enough mana");
        }
    }
    ...
    private void handleCantAttackError(GameModel defender) {
        if (defender instanceof Minion)
            if (((Minion) defender).getAbilities().contains(MinionAbility.STEALTH))
                GameMapper.getInstance().showError("Minion has Stealth!!!");
            else
                GameMapper.getInstance().showError("There is a Taunt on the way!!!");
        else if (gameState.getPlayingModel() instanceof Minion && ((Minion) gameState.getPlayingModel()).getRemainedAttacks() == 0)
            GameMapper.getInstance().showError("Can't Attack with RUSH!!!");
...
//        PlayerLogger.writeLog("Card Burnt", "from deck card : " + card.getName());
        GameMapper.getInstance().getLogger().writeLog(LogType.GAME_REPORT, "Card Burnt -> " + card.getName());
    }
    ...
    public void endGame(boolean interrupted) {
        ...
        writeEvent(LogType.GAME_REPORT, ((interrupted) ? "(Quit-Interrupt)" : ""));
    }
    ...
    private void writeEvent(LogType type, String description) {
        gameState.addEvent(type + ": \n" + description + "\n");
        GameMapper.getInstance().getLogger().writeLog(type, description);
    }
    ...
    private ArrayList<GameModel> getAllModels(Group group) {
        ...
        switch (group) {
            ...
            case PLAYER1:
                ...
                models.add(gameState.getPlayer1().getHero().getWeapon());
                break;
            ...
            case PLAYER2:
                ...
                models.add(gameState.getPlayer2().getHero().getWeapon());
                break;
            case ALL_INCLUDING_DECK:
                models.addAll(getAllModels(Group.PLAYER1_INCLUDING_DECK));
                models.addAll(getAllModels(Group.PLAYER2_INCLUDING_DECK));
                break;
            case ALL:
                models.addAll(getAllModels(Group.PLAYER1));
                models.addAll(getAllModels(Group.PLAYER2));
                break;
        }
